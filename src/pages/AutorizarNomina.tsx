...
const AutorizarNomina = (): React.ReactElement => {
  ...
  const [aprobarNomina, setAprobarNomina] = useState<NominaType>()
  const [view, setView] = useState(false)
  const [employeeSelected, setEmployeeSelected] = useState<EmployeeType>()
  const [visible, setVisible] = useState(false)
  const [visibleAprobarNomina, setVisibleAprobarNomina] = useState(false)
  ...
  useEffect(() => {
    if (nominaRequestStatus === 'success') {
      ...
      form.resetFields()
      setVisible(false)
      setVisibleAprobarNomina(false)
      setAprobarNomina(undefined)
    }
  }, [nominaRequestStatus])
  ...
  const handleSubmit = (record: NominaType) => {
    setVisibleAprobarNomina(true)
    setAprobarNomina(record)
  }
  ...
  const renderItem = (item: NominaType) => {
    return (
      <CustomListItem
        actions={[
          <CustomTooltip key={'edit'} title={'Autorizar Nomina'}>
            <CustomButton
        ...
        <CustomRow justify={'end'}>
          <CustomCol xs={24}>
            <CustomForm form={form}>
              <CustomRow>
                <CustomDivider>
                  <CustomTitle>Autorizar Nomina</CustomTitle>
                </CustomDivider>
              </CustomRow>
              <CustomCollapse
            ...
            <CustomModal
              title={<CustomTitle>Autorizar Nomina</CustomTitle>}
              width={'40%'}
              visible={visibleAprobarNomina}
              cancelText={'Cancelar'}
              onCancel={() => {
                setVisibleAprobarNomina(false)
                form.resetFields()
              }}
              onOk={handleAprobarNomina}
            >
...
export default AutorizarNomina
