...
const Permisos = (): React.ReactElement => {
  ...
  const { fetchingFromEmployee, dataPermissions, dataTypePermissions } =
  ...
  const dataSource = useMemo(() => {
    return filterArray(
      ArrayOrderBy(
        searchInArray(
          dataPermissions?.filter((item) => item.estado === 'A'),
  ...
  }, [dataPermissions, search, searchCargo])
    ...
    return (
      <CustomListItem>
        <CustomListItemMeta
          ...
          description={`Tipo Permiso: ${item.tipo_permiso} Cedula: ${formatter({
        ...
        <CustomSpin spinning={fetchingFromEmployee || fetch}>
          <CustomRow justify={'end'}>
            <CustomCol xs={24}>
              <CustomRow>
                <CustomDivider>
                  <CustomTitle>Reporte de Permisos</CustomTitle>
                </CustomDivider>
